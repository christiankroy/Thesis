...
\section{In the haystack: piRNA precursors}
  ...
  \subsection{Precursor Identity}
    ...
    What determines that a seemingly mRNA-like piRNA precursor is processed into piRNAs, and not translated like crazy by Ribosomes?  Everything meets the ribosome \citep{Guttman2013}, so why is it that precursors are given a different lot in life?
  ...
  \subsection{Precursor Interactions}
    ...
    % From Mihir
      ...
      % Started w/ Review = \citep{Konig2011}
      
      % Clip, Parcip, iClip, etc... (see intro to CLASH paper.


      %  DNA- RNA interactions | Chromatin isolation by RNA purification (ChIRP) = \citep{Chu2012}. Uses DNA probes. RNAse H activity in Crude lysate is a concern. Why it would be good to use RNA probe and 

      % RNA:RNA interactions | RNA Antisense Purification, a technique similar to ChIRP, can be used to look at other RNAs interacting with your target. I think this can also be used identify the proteins in the complex. = \citep{Engreitz2013}
      % RAP. Works well. Concerns are long fixation with glutothion. Captures chromatin. Uses long probes. Means that it would be useful to capturing precursors and not piRNAs, but fixation limits application to sperm cells perhaps? Could also use testes tissue section.
      
      % Finally CLASH = \citep{Helwak2014}
      % CLASH has only been performed in cell culture, and requires a tagged protein. Better genome editing and viral vectors should make this limitation less of barrier, but there still remains the issue of RNA:RNA ligation representing a very low percentage of the reads obtained [This is all my wording]. How would this be applied to the RNA:RNA interactions that piRNA precurors perform?  I suppose you would need to tag MIWI somehow (Did Mourlalotos do this already?). 
  ...
  \subsection{Precursor Imaging}
    ...
    \citep{Lee2014a} % Gold-labeled oligos and FRET
  ...
  \subsection{Precursor Sequencing}
    ...
    % Could periodate treatment could be *somehow* used to selectively-remove piRNAs from the pool of cDNAs that are circularized and sequenced?

    \todo[inline]{Write FISSEQ application to piRNA precursor Tx} % 
    \citep{Ke2013} %previous in situ sequencing.  Mats Nilsson, Professor of Biochemistry and Biophysics at Stockholm University, who in 2013 described a similar in situ transcriptomic approach (2), noted that Church's method is unbiased, whereas Nilsson's used molecular inversion (padlock) probes to amplify specific transcripts.
    \citep{Lee2014a} % FISSEQ 

    %FISSEQ (fluorescent in situ RNA sequencing), which blends the power of RNA FISH and RNA-seq to report the subcellular localization of several thousand genes simultaneously (1).
  ...
  \subsection{Other SeqZip Applications}
    ...
    \subsubsection{SeqZip and single-molecule multi-site FISH}
      ...
      \begin{figure} % Multi-site Fish using SeqZip
        ...
        %Refer to section above using FISSEQ on precursors
...
\section{Final thoughts}
  ...
  \subsection{The Data Deluge}
    ...
    \missingfigure{Graph showing price of storage?}

    % Bring in ideas from the paper that ER sent out about having replicates is better than depth.  Also that packages like DE-Seq gain statistical power when you have more replicates. These additional versions of files will have to be kept seperate, with meta data etc.... Increasing the strain on data analysis and cataloguing (caraling, like animals). 

    %Being able to navigate multiple types of data, ChIP, RNA-Seq, small RNA seq, is essential for observing important aspects of Biology. There is no replacement for visuazliation, and efforts such as ($REF Integrated genomeics viewer, UCSC Genome broweser, and TrackHUBS paper$ demonstrate how important this is, but places additional strain on the required systems.

    % Efforts such as Galaxy ($REF) are great starts, but need to be more cross diciplanary, and locally deployed <This sounds like BS, but some idea is there>. 
  ...
  \subsection{Biologists need Computation Biological Skills}
    ...
    \missingfigure{Table showing what people use vs. what they should use}
